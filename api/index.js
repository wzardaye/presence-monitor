...
// Vercel Serverless best practice: Cache the connection so it doesn't reconnect on every click
...
async function connectToDatabase() {
  ...
  const db = client.db('presence_monitor_db'); // Name of your database
...
async function readData() {
  try {
    ...
    // We store everything in a single document with the ID "main_data" for simplicity
    const data = await collection.findOne({ documentId: 'main_data' });
    if (data) return data;

    // First Run Initialization: If Mongo is empty, read default data.json and upload it
...
async function writeData(data) {
  ...
  try {
    ...
    // Upsert (Update or Insert) the document in MongoDB
...
app.post(['/api/presence', '/presence'], async (req, res) => {
  // Grab the reason from the frontend request
  ...
  if (person) {
    ...
    // Save the reason if absent, or clear it if they check back in
    if (!present) {
      person.reason = reason || "Unspecified";
    } else {
      person.reason = ""; 
    }
    ...
    person.logs.push({
      ...
      action: present ? "Checked IN" : `Checked OUT (Reason: ${person.reason})`
    });
...
// MongoDB Connection Setup
// Vercel Serverless best practice: Cache the connection so it doesn't reconnect on every request
const MONGODB_URI = process.env.MONGODB_URI;
let cachedClient = null;
let cachedDb = null;
